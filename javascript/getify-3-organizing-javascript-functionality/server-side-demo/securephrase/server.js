...
global.Request = Request = require(path.join(
  __dirname,
  "web",
  "js",
  "Request.js"
));
global.RandomIntegers = RandomIntegers = require(path.join(
  __dirname,
  "web",
  "js",
  "RandomIntegers.js"
));
global.WordList = WordList = require(path.join(
  __dirname,
  "web",
  "js",
  "WordList.js"
));
global.RandomPhrase = RandomPhrase = require(path.join(
  __dirname,
  "web",
  "js",
  "RandomPhrase.js"
...
routes.push(
  ...
  function securityHeaders(req, res) {
    ...
    res.setHeader(
      ...
      "default-src 'self'; script-src 'self' 'unsafe-eval' localhost:8050 ajax.googleapis.com ssl.google-analytics.com; connect-src 'self' www.random.org; style-src 'self' 'unsafe-inline' localhost:8050"
    );

    // From: https://developer.mozilla.org/en-US/docs/Security/HTTP_Strict_Transport_Security
    res.setHeader(
      "Strict-Transport-Security",
      "max-age=" + 1e9 + "; includeSubdomains"
    );
  }
);
...
routes.push(
  ...
  function staticResources(req, res) {
    if (
      req.method === "GET" &&
      /^\/(?:js\/(?=.+)|css\/(?=.+)|images\/(?=.+)|robots\.txt\b|humans\.txt\b|favicon\.ico\b)/.test(
        req.url
      )
    ) {
      req.url = "/web" + req.url;
...
routes.push(
  ...
  function loadPage(req, res) {
    ...
    if (req.method === "GET" && (url = Pages.recognize(req.url))) {
...
function* router(token) {
  var req = token.messages[0],
    res = token.messages[1],
    route,
    error;
...
function responseError(respErr) {
  ...
  ERROR(
    "responseError",
    respErr
      ? respErr.stack + "" || respErr.toString()
