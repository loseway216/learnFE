    ...
    if (st.isFile() && /\.grips\.html$/.test(file)) {
      ...
      try {
        bundle_str += grips.compileCollection(
          /*source=*/ contents,
          /*collectionID=*/ collectionPrefix +
            "/" +
            file.replace(/(?:\.?index)?\.grips\.html$/, ""),
          /*initialize=*/ false
        );
      } catch (err) {
...
function buildTemplates() {
  ...
  var bundle_wrapper = fs.readFileSync(
    path.join(__dirname, "templates", "tmpls-wrapper.js"),
    { encoding: "utf8" }
  );

  bundle_wrapper = bundle_wrapper.replace(/\/\*TEMPLATES\*\//, function () {
    return bundle_str;
  });
